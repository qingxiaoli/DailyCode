...
function mosaic = two_image_mosaic(img1, img2)
% coder: Jie An
% version: 20170405
...
img1_m = zeros(y_r - y_l + 1, x_r - x_l + 1);
img2_m = zeros(y_r - y_l + 1, x_r - x_l + 1);
img1_m(position(1, 1): position(1, 1) + RB.ImageSize(1) - 1, position(1, 2): position(1, 2) + RB.ImageSize(2) - 1) = img1_t;
img2_m(position(2, 1): position(2, 1) + size(img2, 1) - 1, position(2, 2): position(2, 2) + size(img2, 2) - 1) = img2;
