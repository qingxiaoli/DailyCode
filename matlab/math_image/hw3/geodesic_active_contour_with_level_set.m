...
for i = 1 : MAX_ITERATION
    ...
    if mod(i, 50) == 1
        subplot(121), imagesc(img_noise), axis image off, hold on,
        contour(v, [0, 0], 'k'), drawnow, hold off;
        title('zero level set result on image');
        subplot(122), imagesc(v), axis image off, hold on,
        contour(v, [0, 0], 'k'), drawnow, hold off;
        title('zero level set result on level set function');
