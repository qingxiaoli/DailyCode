...
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=Y_pre, \
    labels=Y))
...
with tf.Session() as sess:
    ...
    for i in range(100):
        for start, end in zip(range(0, len(train_x), 100), range(100, \
            len(train_x) + 1, 100)):
            sess.run(train_op, feed_dict={X: train_x[start: end], Y: \
                train_y[start: end]})
